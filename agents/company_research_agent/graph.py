...
from agents.company_research_agent.state import OverallState, InputState, OutputState
from agents.company_research_agent.configuration import Configuration
from agents.company_research_agent.utils import (
    deduplicate_and_format_sources,
    format_all_notes,
)
from agents.company_research_agent.prompts import (
    QUERY_WRITER_PROMPT,
    INFO_PROMPT,
    EXTRACTION_PROMPT,
    REFLECTION_PROMPT,
...
model = ChatOpenAI(model="gpt-4o", rate_limiter=rate_limiter, temperature=0)
...
def gather_notes_extract_schema(state: OverallState) -> dict[str, Any]:
    ...
    system_prompt = EXTRACTION_PROMPT.format(
        schema=json.dumps(state.extraction_schema, indent=2), notes=notes
    )
...
def reflection(state: OverallState) -> dict[str, Any]:
    ...
    system_prompt = REFLECTION_PROMPT.format(
        schema=json.dumps(state.extraction_schema, indent=2), info=state.info
    )
...
workflow = StateGraph(
    OverallState, input=InputState, output=OutputState, config_schema=Configuration
)
